...
app.get("/api/tasks/:id", (req, res) => {
  const task = ALL.find((item) => item.id === req.params.id);
  if (!task) {
...
wss.on("connection", (ws) => {
  ...
  const timer = setInterval(() => {
    ...
    const events = [
      { kind: "task.updated", payload: { id: task.id, status: STATUSES[n % STATUSES.length], updatedAt: Date.now() } },
      { kind: "task.assigned", payload: { id: task.id, assignee: USERS[n % USERS.length] } },
      { kind: "annotation.created", payload: { taskId: task.id, by: "u1", at: Date.now() } },
