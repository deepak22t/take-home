...
function PageSizeSelect({ value, onChange }: { value: number; onChange: (value: number) => void }) {
  ...
  return (
    <div ref={containerRef} className="relative min-w-[4.25rem]">
      ...
      {isOpen ? (
        <div className="absolute right-0 top-[calc(100%+0.35rem)] z-30 min-w-full overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-[0_16px_40px_rgba(15,23,42,0.14)]">
          <ul role="listbox" aria-label="Page size" className="scrollbar-hidden max-h-56 overflow-y-auto">
            {options.map((size) => {
