...
function FilterSelect({
  ...
  return (
    <div ref={containerRef} className="relative min-w-0 text-sm text-slate-700">
      ...
      {isOpen ? (
        <div className="absolute left-0 right-0 top-[calc(100%+0.35rem)] z-30 overflow-hidden rounded-xl border border-slate-200 bg-white p-1 shadow-[0_12px_28px_rgba(15,23,42,0.12)]">
          <ul role="listbox" aria-label={label} className="scrollbar-hidden max-h-44 overflow-y-auto">
            {options.map((option) => {
